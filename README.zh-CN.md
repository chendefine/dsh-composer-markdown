...
| 层级缩进（升级/取消） | 同上位置按 **Shift+Tab**，或折叠光标恰在**前缀原子之后**（内容首）按 **Backspace** | 嵌套项（缩进 ≥1 空格）→ 该项及全部子层级各 −2 空格（奇数缩进按 `min(2, 缩进)` 取整到 0）；**顶层项 → 取消列表**：删除整个前缀原子，只留正文成普通文本行，子层级仍各升一级。取消列表留下普通行会隔断有序组，其后同缩进成员按不变式从 `1.` 重起。长按 Tab 降一级后停在层级上限（认领但不动）；长按 Shift+Tab 每次重复升一级——两键焦点都不会从列表行跳走 |
